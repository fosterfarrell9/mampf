...
$('#new-seminar-area').empty()
  .append('<%= j render partial: "seminars/new",
                        locals: { seminar: @seminar,
                                  from: @from } %>').show()
$('#new-seminar-area .selectize').selectize({ plugins: ['remove_button'] })
...
teacherSelector = document.getElementById('seminar_teacher_id')
